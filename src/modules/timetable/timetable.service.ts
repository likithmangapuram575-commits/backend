...
export const getTimetableByDepartment = async (departmentId: number) => {
...
export const checkConflict = async (data: any) => {
  // Check if faculty is busy at this time (by day/period OR time range)
  const [facultyConflict]: any = await pool.query(`
  ...
  const [sectionConflict]: any = await pool.query(`
    SELECT * FROM timetable 
...
export const generateTimetable = async (batchId: number, year: number, semester: number) => {
  ...
  try {
    ...
    const [sections]: any = await connection.query(
      'SELECT id, name, department_id FROM sections WHERE batch_id = ? AND year = ?',
      [batchId, year]
    );

    if (sections.length === 0) throw new Error('No sections found for this batch/year');
    ...
    if (assignments.length === 0) throw new Error('No subject assignments found for this batch/year/semester');

    // 3. Clear existing timetable for these sections
    const sectionIds = sections.map((s: any) => s.id);
    await connection.query('DELETE FROM timetable WHERE section_id IN (?)', [sectionIds]);
    ...
    const facultyDailyLoad: any = {};
    const weeklyTracker: any = {}; // { sectionId_subjectId: count }

    const [existingSchedules]: any = await connection.query(
      'SELECT faculty_id, day, period FROM timetable WHERE day IS NOT NULL AND period IS NOT NULL'
    );
    
    existingSchedules.forEach((row: any) => {
      if (!facultySchedule[row.faculty_id]) facultySchedule[row.faculty_id] = {};
      if (!facultySchedule[row.faculty_id][row.day]) facultySchedule[row.faculty_id][row.day] = {};
      facultySchedule[row.faculty_id][row.day][row.period] = true;
      
      if (!facultyDailyLoad[row.faculty_id]) facultyDailyLoad[row.faculty_id] = {};
    ...
    for (const day of days) {
      const filledPeriods: any = {};
      sections.forEach((s: any) => filledPeriods[s.id] = new Set());

      // 5a. LABS (1x/week, 3 slots)
      for (const section of sections) {
        const labs = assignments.filter((a: any) => a.section_id === section.id && a.subject_name.toLowerCase().includes('lab'));
        for (const lab of labs) {
          const trackerKey = `${section.id}_${lab.subject_id}`;
          if ((weeklyTracker[trackerKey] || 0) >= 3) continue;

          const [anyLabToday]: any = await connection.query(
            'SELECT t.id FROM timetable t JOIN subjects s ON t.subject_id = s.id WHERE t.section_id = ? AND t.day = ? AND s.name LIKE "%Lab%"',
            [section.id, day]
          );
          if (anyLabToday.length > 0) continue;

          const possibleStarts = [1, 2, 5]; 
          for (const start of possibleStarts.sort(() => Math.random() - 0.5)) {
            const range = [start, start + 1, start + 2];
            if (range.some(p => p > 7)) continue; 

            const isFree = range.every(p => !filledPeriods[section.id].has(p) && !facultySchedule[lab.faculty_id]?.[day]?.[p]);

            if (isFree && (facultyDailyLoad[lab.faculty_id]?.[day] || 0) + 3 <= MAX_PERIODS_PER_DAY) {
              for (const p of range) {
                await insertEntry(connection, section, lab, day, p);
      ...
      for (const period of shuffledPeriods) {
        for (const section of sections) {
          if (filledPeriods[section.id].has(period)) continue;

          const theoryAssignments = assignments.filter((a: any) => 
            a.section_id === section.id && !a.subject_name.toLowerCase().includes('lab')
          ).sort((a: any, b: any) => {
          ...
          for (const assignment of theoryAssignments) {
            ...
            if ((weeklyTracker[trackerKey] || 0) >= 6 && Math.random() > 0.2) continue; 

            if (facultySchedule[assignment.faculty_id]?.[day]?.[period]) continue;
            if ((facultyDailyLoad[assignment.faculty_id]?.[day] || 0) >= MAX_PERIODS_PER_DAY) continue;

            const [subjectToday]: any = await connection.query(
              'SELECT id FROM timetable WHERE section_id = ? AND day = ? AND subject_id = ?',
              [section.id, day, assignment.subject_id]
            );
            if (subjectToday.length > 0) continue;

            await insertEntry(connection, section, assignment, day, period);
...
const insertEntry = async (connection: any, section: any, assignment: any, day: number, period: number) => {
  const getTimes = (p: number) => {
    const slots: any = {
...
const markFacultyBusy = (facultySchedule: any, facultyDailyLoad: any, facultyId: number, day: number, period: number) => {
  ...
  if (!facultyDailyLoad[facultyId]) facultyDailyLoad[facultyId] = {};
  facultyDailyLoad[facultyId][day] = (facultyDailyLoad[facultyId][day] || 0) + 1;
};

export const deleteTimetableEntry = async (id: number) => {
